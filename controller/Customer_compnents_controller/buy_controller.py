...
class BuyController:
    def __init__(self, root, supplier):
        ...
        self._bind_events()
        self.view.populate_treeview(self.model.get_buys_from_db(self.supplier))
    ...
    def save_buys(self):
        ...
        if self.model.insert_buys_to_db(self.view.temp_operations, self.supplier):
    ...
    def cache_buy(self):
        if self.view.check_inputs_before_caching(customers_names = self.model.get_customer_names_and_money(self.supplier),products_codes = self.model.get_products_codes()): # validation input 
            data = [ # cus_name, productcode, price, quantity, discount, supplier , cash or not
                self.view.cus_name.get(), self.view.product_code.get(), self.view.price.get(), self.view.quantity.get(), self.view.discount.get(), self.view.checkbox_var.get()
                ]
            ...
            for ent in [self.view.cus_name, self.view.product_code, self.view.price, self.view.quantity, self.view.discount]:
    ...
    def recommendation_focusIn(self, ent, var):
        ...
        if ent == self.view.Entries[0]:
            ...
            self.view.recommendations = self.model.get_customer_names_and_money(self.supplier)
