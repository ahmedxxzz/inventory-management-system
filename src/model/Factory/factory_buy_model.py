...
class FactoryBuyModel:
    ...
    def save_purchase_bill(self, bill_data):
        """
        Saves the entire purchase bill and returns a dictionary with the new bill's ID
        and the factory's balance before and after the transaction.
        """
        cursor = self.conn.cursor()
        try:
            ...
            if bill_data['is_paid'] == 0: # Only add to balance if the bill is on credit
                factory_balance_after += bill_data['total_amount']

            # ... (باقي منطق الحفظ الداخلي يبقى كما هو)
            cursor.execute("""
            ...
            for item in bill_data['items']:
                ...
                if product_row:
                    ...
                    if not dist_row: raise ValueError(f"الموزع '{item['distributor_name']}' غير موجود.")
                    dist_id = dist_row[0]
                ...
                else: new_wac = new_price

                cursor.execute("""
                    UPDATE Product SET available_quantity = available_quantity + ?, weighted_average_cost = ? WHERE product_id = ?
            ...
            if bill_data['is_paid'] == 0:
                 cursor.execute("""
                    UPDATE Factories SET current_balance = ?, current_quantity = current_quantity + ?, last_purchase_date = ? WHERE factory_id = ?
                """, (factory_balance_after, total_quantity_purchased, bill_data['date'], factory_id))
            else:
                 cursor.execute("""
                    UPDATE Factories SET current_quantity = current_quantity + ?, last_purchase_date = ? WHERE factory_id = ?
                """, (total_quantity_purchased, bill_data['date'], factory_id))

            self.conn.commit()
            
            # --- MODIFIED: Return a dictionary with all necessary info ---
            return {
                'id': purchases_bill_id,
