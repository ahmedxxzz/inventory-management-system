...
class CustomerAccountDetailsView(ctk.CTkFrame):
    def __init__(self, root, customer_name):
        """create the customer account details frame and its commponents inside the customer main frame
    ...
    def create_upper_frame(self):
        ...
        self.name_lbl = ctk.CTkLabel(name_frame, text=f'مكتب: {self.customer_name}', font=("Arial", 20, "bold"), text_color='white',width=200, height=40)
    ...
    def create_edit_name_popup(self, current_name):
        """Creates a popup to edit the customer name.
        
        Args:
            current_name: The current customer name to pre-fill
            
        Returns:
            str or None: The new name if saved, None if cancelled
        """
        popup = ctk.CTkToplevel(self)
        popup.title("تعديل اسم المكتب")
        popup.grab_set()
    ...
    def update_name_label(self, new_name):
        """Updates the displayed customer name."""
        self.customer_name = new_name
        self.name_lbl.configure(text=f'مكتب: {new_name}')
