...
class FactoryReturnView(ctk.CTkFrame):
    def __init__(self, root):
        ...
        ctk.CTkLabel(top_frame, text=":المصنع", font=self.main_font).grid(row=0, column=1, padx=(10, 20), pady=10, sticky="e")
        self.factory_combobox = ctk.CTkComboBox(top_frame, values=[], state="readonly", justify='right', font=self.main_font)
        self.factory_combobox.grid(row=0, column=0, padx=(20, 10), pady=10, sticky="ew")
        ...
        # <<<--- REQUIREMENT #1: CENTERING TABLE CONTENT AND HEADERS --- START --->
        self.tree = ttk.Treeview(table_frame, columns=("total", "price", "quantity", "product"), show="headings")
        self.tree.heading("product", text="الصنف", anchor='center') # Header centered
        ...
        self.tree.column("product", anchor='center', width=250) # Content centered
        ...
        # <<<--- REQUIREMENT #1: CENTERING TABLE CONTENT AND HEADERS --- END --->
